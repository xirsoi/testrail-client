...
namespace TestRail
{
    ...
    public class TestRailClient
    {
		...
		/// <summary>
		/// The http client used by this object. Persisting it prevents exhausting our pool of ports.
		/// </summary>
        ...
        public class HTTPRequestSentEventArgs : EventArgs
        {
            ...
            public HTTPRequestSentEventArgs(HttpMethod method, Uri uri, string postContent = null)
            {
                Method = method;
                Uri = uri;
        ...
        public TestRailClient(string url, string username, string password)
        {
			...
			// build http client
			_Client.BaseAddress = new UriBuilder(url).Uri;
			_Client.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("TestRail Client for .Net Standard", "1.4"));
			_Client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
			var authInfo = Convert.ToBase64String(Encoding.UTF8.GetBytes($"{_UserName_}:{_Password_}"));
        ...
        {
            var result = _CallTestRailGetEndpoint(uri).Result;
            if (!result.WasSuccessful)
            {
        ...
        private async Task<CommandResult> _CallTestRailGetEndpoint(string uri)
        {
            var realUri = new UriBuilder(_URL_ + uri).Uri;
            OnHTTPRequestSent(this, new HTTPRequestSentEventArgs(HttpMethod.Get, realUri));
            CommandResult cr;
            try
            {
                ...
                cr = new CommandResult(response.StatusCode == HttpStatusCode.OK, responseContent);
